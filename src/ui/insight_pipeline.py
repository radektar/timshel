...
def latest_deck() -> Optional[im.InsightDeck]:
    ...
    # U5: quiet provenance metadata — "digest · dd.mm"; the window appends
    # the tool name ("Claude"). The word "chmura" is banned on this surface.
    # Date parsed from the digest note filename ("YYYY-MM-DD Synthesis[…].md").
    ...
    if digest_name[:10].count("-") == 2:
        ...
        if m.isdigit() and d.isdigit():
            label = f"digest · {d}.{m}"
    deck.digest_label = label or "digest"
