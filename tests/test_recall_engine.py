...
class FakeEmbedder:
    ...
    def _vec(self, text):
        ...
        for raw in text.lower().replace("\n", " ").split():
            ...
            if tok:
                v[
                    int(hashlib.md5(tok.encode("utf-8")).hexdigest(), 16) % self.dim
                ] += 1.0
...
def test_dim_change_rebuilds_store(vault, monkeypatch):
    monkeypatch.setattr(
        engine_mod, "resolve_embedder", lambda *a, **k: FakeEmbedder(32)
    )
    ...
    monkeypatch.setattr(
        engine_mod, "resolve_embedder", lambda *a, **k: FakeEmbedder(48)
    )
...
def test_cli_backfill_then_ask(tmp_path, monkeypatch, capsys):
    ...
    _note(
        ...
        "Okna",
        "14.06",
        "Producenci okien nie odpowiadaja, dach stoi w miejscu.",
    )
...
def test_backfill_incremental_skips_unchanged_reindexes_edited(vault, monkeypatch):
    ...
    try:
        ...
        assert (
            eng.backfill(incremental=True) == 0
        )  # nothing changed → nothing reindexed
