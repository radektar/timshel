...
def test_save_answer_writes_under_recall_subdir(tmp_path):
    path = aw.save_answer(
        "co z oknami i dachem?", _answer(), tmp_path, date_str="26-07-01"
    )
    assert path.exists()
...
def test_frontmatter_survives_quotes_and_newlines():
    ...
    md = aw.render_answer_md(
        'czy on powiedzial "nie"? \\ i nowa\nlinia', _answer(), date_str="26-07-01"
    )
    fm = md.split("---")[1]
    parsed = yaml.safe_load(
        fm
    )  # must not raise on quote/backslash/newline in the query
