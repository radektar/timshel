...
- **✅ SHIPPED (app) 2026-07-01** — `enable_recall_index` defaults **True**; on launch a daemon thread runs `engine.backfill(incremental=True)` (skips notes unchanged by content hash via `store.note_version`), non-blocking. Thread-safe `IndexingState` (Standby/Indexing/Ready/Error + progress) drives a partial-index banner in the recall view. In-app privacy disclosure states search=local, cloud only on explicit "Zsyntetyzuj". Conftest isolates the flag off in the suite. 800 tests. **🟡 OPEN — landing privacy copy:** the landing over-claims blanket "nic nie wychodzi / nigdy nie opuszczają Maca" (site/index.html:490,526,546,567) while insights/synthesis (BYOK) DO send text to the cloud. Needs a precise reframe (transkrypcja+search=lokalnie; insights=Twój klucz, minimalny tekst, na Twoją akcję). Deferred: it's a product-copy decision on a file the teaser branch owns.
